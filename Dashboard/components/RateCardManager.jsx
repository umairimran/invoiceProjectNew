...
const RateCardManager = ({ client, onUploadRateCard }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (success) {
        setSelectedFile(null);
        // Reset file input
        const fileInput = document.getElementById('rate-card-upload');
  ...
  const handleDownload = () => {
    if (!client.rate_card_file) return;
    
    // Extract filename from path if it's a full path
    const filename = client.rate_card_file.split('/').pop();
  ...
  return (
    <div className="card">
      <h2 className="font-signika font-bold text-xl mb-6">Rate Card Template</h2>
        ...
        <div className={`p-6 rounded-xl border-2 ${
          ...
          <div className="flex items-center">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mr-6 ${
              client.rate_card_file ? 'bg-green-100' : 'bg-gray-100'
            }`}>
              <i className={`fas fa-${client.rate_card_file ? 'check-circle text-green-600' : 'file-alt text-gray-500'} text-2xl`}></i>
            </div>
            <div className="flex-1">
              <h3 className={`text-xl font-bold mb-2 ${
                client.rate_card_file ? 'text-green-800' : 'text-gray-700'
              }`}>
                {client.rate_card_file ? 'Template Available' : 'No Template Uploaded'}
              </h3>
              {client.rate_card_file ? (
                <div className="space-y-2">
                  <div className="flex items-center text-green-700">
                    <i className="fas fa-file-excel mr-2"></i>
                    <span className="font-medium">{client.rate_card_file.split('/').pop()}</span>
                  </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
            ...
            <div className="flex items-center mb-3">
              ...
              <div>
                <h4 className="font-semibold text-gray-900 text-lg">
                  {client.rate_card_file ? 'Replace Template' : 'Upload Template'}
                </h4>
                <p className="text-gray-600 text-sm">
                  {client.rate_card_file ? 'Upload a new file to replace the current template' : 'Choose your rate card template file'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
              <label htmlFor="rate-card-upload" className="cursor-pointer">
